...
public class LoadoutRemove extends Command{
	...
	public CommandResponse execute(Player player, Set<String> flags, Map<String, Object> parameters, List<Object> baseParameters)
		...
		if (!CVPvp.getInstance().getLoadoutManager().contains((String) baseParameters.get(0)))
			throw new CommandExecutionException("&cLoadout &6" + baseParameters.get(0).toString().toLowerCase() + "&c does not exist!");
		
		if (parameters.containsKey("team")) {
			LoadoutContainer lc = CVPvp.getInstance().getLoadoutManager().getLoadoutByName((String) baseParameters.get(0));
			...
			CVPvp.getInstance().getLoadoutManager().removeLoadout((String) baseParameters.get(0));
