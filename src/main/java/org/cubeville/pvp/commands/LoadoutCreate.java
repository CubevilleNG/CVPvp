...
public class LoadoutCreate extends Command {
    ...
    public CommandResponse execute(Player player, Set<String> flags, Map<String, Object> parameters, List<Object> baseParameters)
        ...
        if (!parameters.containsKey("team")) {
            if (!CVPvp.getInstance().getLoadoutManager().createLoadout(player, (String) baseParameters.get(0))) {
        ...
        } else if (CVPvp.getInstance().getLoadoutManager().getLoadoutByName((String) baseParameters.get(0)) != null) {
            CVPvp.getInstance().getLoadoutManager().getLoadoutByName((String) baseParameters.get(0)).createInventory(player, (String) parameters.get("team"));
